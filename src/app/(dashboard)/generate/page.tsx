...
function EmptyState({ onGenerate }: { onGenerate: () => void }) {
  return (
    <div className="flex flex-col items-center justify-center flex-1 py-20 px-6 text-center">
      ...
      <div className="flex flex-wrap gap-2 justify-center mt-10 opacity-40">
        {["High Protein", "Vegetarian", "Gluten-Free", "Dairy-Free", "Quick Meals"].map((t) => (
          <span
            key={t}
            className="text-xs px-3 py-1 rounded-full border border-green-600 text-green-700 font-medium"
          >
            {t}
          </span>
        ))}
      ...
      <div className="flex-1 flex flex-col max-w-4xl w-full mx-auto px-8 py-8">
        ...
        {status === "error" && (
          <div className="flex flex-col items-center justify-center flex-1 py-24 text-center">
            ...
            <p className="text-sm text-gray-400 mb-6 max-w-xs">{errorMsg}</p>
            <button
              onClick={handleGenerate}
